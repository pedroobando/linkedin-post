/**
 * API Endpoint: Sign In User
 *
 * Este endpoint permite iniciar sesión a un usuario existente en el sistema.
 * Devuelve un token JWT que puede ser usado para autenticación en APIs externas.
 *
 * @usage
 * POST /api/auth/signin-user
 *
 * @headers
 * Authorization: Bearer {ADMIN_API_TOKEN}
 * Content-Type: application/json
 *
 * @body
 * {
 *   "email": "usuario@ejemplo.com",
 *   "password": "contraseña123"
 * }
 *
 * @responses
 * 200: {
 *   "message": "User signed in successfully",
 *   "user": {
 *     "id": "user_id_generado",
 *     "createdAt": "2023-01-01T00:00:00.000Z",
 *     "updatedAt": "2023-01-01T00:00:00.000Z",
 *     "email": "usuario@ejemplo.com",
 *     "emailVerified": false,
 *     "name": "Nombre del Usuario",
 *     "role": "user"
 *   },
 *   "token": "token_de_sesion",
 *   "jwtToken": "token_jwt_para_api_externas"
 * }
 *
 * 400: {
 *   "error": "Missing required fields: email, password"
 * }
 *
 * 401: {
 *   "error": "Invalid token" | "Error signing in user"
 * }
 *
 * 500: {
 *   "error": "Internal server error"
 * }
 *
 * @example
 * curl -X POST http://localhost:3000/api/auth/signin-user \
 *   -H "Authorization: Bearer tu_token_admin_aqui" \
 *   -H "Content-Type: application/json" \
 *   -d '{
 *     "email": "juan@example.com",
 *     "password": "contraseña123"
 *   }'
 *
 * @note
 * Este endpoint requiere un token de administrador para autenticación.
 * Además de la información de sesión estándar, devuelve un token JWT
 * que puede ser usado para autenticación en APIs externas.
 */

import { NextRequest } from 'next/server';
import { signInAction } from '@/actions/auth/signin-action';
import { generateJWTToken } from '@/lib/generate-jwt';

// Función para verificar el token de autorización
const verifyToken = (request: NextRequest): boolean => {
  const authHeader = request.headers.get('authorization');
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    return false;
  }

  const token = authHeader.substring(7); // Remover 'Bearer ' del principio
  const adminToken = process.env.ADMIN_API_TOKEN;

  return token === adminToken;
};

export async function POST(req: NextRequest) {
  // Verificar el token de autorización
  if (!verifyToken(req)) {
    return Response.json(
      {
        success: false,
        message: 'Token de autorización inválido o ausente.',
      },
      { status: 401 },
    );
  }

  try {
    // Parse the request body
    const userData = await req.json();

    // Validate required fields
    if (!userData.email || !userData.password) {
      return Response.json({ error: 'Missing required fields: email, password' }, { status: 400 });
    }

    // Use the server action to sign in the user
    const [result, error] = await signInAction({
      email: userData.email,
      password: userData.password,
    });

    if (error) {
      console.error('Error signing in user:', error);
      return Response.json(
        { error: error.message || 'Error signing in user' },
        { status: 401 }, // Unauthorized status for login failures
      );
    }

    // Generar un token JWT para uso en APIs externas
    const jwtToken = await generateJWTToken(
      {
        userId: result?.user.id,
      },
      '2d',
    ); // 2 días de expiración

    // Return success response
    return Response.json(
      {
        message: 'User signed in successfully',
        user: result?.user,
        token: result?.token,
        jwtToken: jwtToken,
      },
      { status: 200 },
    );
  } catch (error) {
    console.error('Error signing in user:', error);
    return Response.json({ error: 'Internal server error' }, { status: 500 });
  }
}
